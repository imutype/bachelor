...
*/
/**************************************************************************/
Adafruit_BNO055::Adafruit_BNO055(TwoWire& bus_, int32_t sensorID, uint8_t address)
    : bus(bus_) {
    _sensorID = sensorID;
    _address = address;
}
...
#ifdef ESP8266
    bus.setClockStretchLimit(1000); // Allow for 1000us of clock stretching
#endif
...
void Adafruit_BNO055::getQuatRaw(char* buffer) {
    memset(buffer, 0, 8);
    readLen(BNO055_QUATERNION_DATA_W_LSB_ADDR, (byte*)buffer, 8);
}
void Adafruit_BNO055::getAccRaw(char* buffer) {
    memset(buffer, 0, 6);
    readLen(BNO055_LINEAR_ACCEL_DATA_X_LSB_ADDR, (byte*)buffer, 6);
}
void Adafruit_BNO055::getGyroRaw(char* buffer) {
    memset(buffer, 0, 6);
    readLen(BNO055_GYRO_DATA_X_LSB_ADDR, (byte*)buffer, 6);
}
...
#endif
    bus.endTransmission();
    bus.requestFrom(_address, (byte)1);
#if ARDUINO >= 100
    value = bus.read();
#else
    value = bus.receive();
...
#endif
    bus.endTransmission();
    bus.requestFrom(_address, (byte)len);

    for (uint8_t i = 0; i < len; i++) {
#if ARDUINO >= 100
        buffer[i] = bus.read();
#else
        buffer[i] = bus.receive();
#endif
    }
